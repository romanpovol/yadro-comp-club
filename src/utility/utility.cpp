...
my_time::my_time(std::int32_t hours, std::int32_t minutes) :
        ...
        m_hours(hours + minutes / 60) {}


std::string my_time::to_string() const {
    if (!(0 <= m_hours && m_hours <= 23 && 0 <= m_minutes && m_minutes <= 59)) {
        throw std::domain_error("time is out of bounds: m_hours: " + std::to_string(m_hours) + ", m_minutes: " +
                                std::to_string(m_minutes));
    }
...
std::int32_t my_time::minutes() const {
    return m_minutes;
}

std::int32_t my_time::hours() const {
    return m_hours;
}

std::int32_t my_time::rounded_up_number_of_hours() const {
    return m_hours + (m_minutes != 0);
}
